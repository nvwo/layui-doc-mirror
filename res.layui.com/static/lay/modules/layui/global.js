...
layui.define(['code', 'element', 'table', 'util'], function(exports){
  ...
  layer.ready(function(){
    ...
    ;!function(){
      ...
      ,elemParterTpl = $('#layui-tpl-parter')
      ,optionsParter = (function(){
        try {
          return new Function('return '+ (elemParterTpl.attr('lay-options') || '{}'))()
        } catch(e) {
      ...
      var notParter = local.notice_parter && new Date().getTime() - local.notice_parter < (optionsParter.times || 1000*60*30); //默认 30 分钟出现一次

      if(elemParter[0] && !notParter && elemParter.data('tips') == 1){
        var tipsIndex = layer.tips(
          ['<a href="https://fly.layui.com/jump/'+ (optionsParter.type || 'alyhot') +'/" target="_blank" id="layui-spm-event-parter-tips" style="display: block; line-height: 30px; padding: 10px; text-align: center; font-size: 14px; background-image: linear-gradient(to right,#8510FF,#D025C2,#FF8B2D,#FF0036); color: #fff; '+ (optionsParter.style || '') +'">' 
          ...
          ,{
            tips: (optionsParter.tipsStyle || [3, '#9F17E9'])
  ...
  ,runCodes = function(){
    ...
    demoForm.action = '/api/runHtml.html';
    demoForm.submit();
